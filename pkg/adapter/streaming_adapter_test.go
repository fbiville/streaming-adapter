...
var _ = Describe("Streaming adapter", func() {
	...
	Describe("when given proper args", func() {
		var (
			...
			grpcAddress      string
			streamingAdapter *adapter.StreamingAdapter
	...
	Describe("when given wrong arguments", func() {
		var streamingAdapter *adapter.StreamingAdapter
		...
		It("refuses to start on a busy HTTP port", func() {
			...
			err = streamingAdapter.Start(busyPort)
		...
		It("returns 5xx errors when the gRPC server is unreachable", func() {
			...
			streamingAdapter = &adapter.StreamingAdapter{
				ServiceResolver: &HardcodedKoResolver{},
				Timeout:         timeout,
			}
			err := streamingAdapter.Start(port)
			Expect(err).NotTo(HaveOccurred())
			defer assertClose(streamingAdapter)
			httpClient := &http.Client{}
		...
		It("returns 5xx errors when the gRPC server fails to be invoked", func() {
			port := findFreePort()
			adapterAddress := fmt.Sprintf("http://localhost:%d", port)
			grpcConnection, grpcAddress := openGrpcConnection(ErroringFrenchizerServer(func(int) error {
		...
		It("returns 5xx errors only for failing invocations", func() {
			port := findFreePort()
			adapterAddress := fmt.Sprintf("http://localhost:%d", port)
			grpcConnection, grpcAddress := openGrpcConnection(ErroringFrenchizerServer(func(invocationCount int) error {
...
type HardcodedKoResolver struct{}

func (*HardcodedKoResolver) Resolve(*http.Request) (*grpc.ClientConn, error) {
	return grpc.Dial(fmt.Sprintf("localhost:%d", findFreePort()), grpc.WithInsecure())
}
