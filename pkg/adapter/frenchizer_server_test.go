package adapter_test // visible for tests only
...
func (frenchizer *frenchizerServer) Invoke(server streaming.Riff_InvokeServer) error {
...
func (frenchizer *frenchizerServer) onNext(accept *string, value *streaming.Next) (string, error) {
	if accept == nil {
...
func nextSignal(payload string) *streaming.Signal {
	value := streaming.Signal_Next{Next: &streaming.Next{Payload: []byte(payload)}}
	return &streaming.Signal{Value: &value}
}
